-- P0 FIX: Update calculate_full_health to exclude paid fixed expenses and investments
-- This ensures the health calculation matches the breakdown display
...
BEGIN
  ...
  -- P0 FIX: Total monthly fixed expenses EXCLUDING paid ones for current billing period
  SELECT COALESCE(SUM(
    CASE fe.frequency
  ...
  WHERE fe.user_id = p_user_id
    -- Exclude expenses that are paid in current billing period
    AND NOT EXISTS (
  ...
  -- P0 FIX: Total monthly investments (active only) EXCLUDING paid ones for current billing period
  SELECT COALESCE(SUM(inv.monthly_amount), 0) INTO v_total_investments
  FROM investments inv
  WHERE inv.user_id = p_user_id
    AND inv.status = 'active'
    -- Exclude investments that are paid in current billing period
    AND NOT EXISTS (
      SELECT 1 FROM payments p
  ...
  RETURN json_build_object(
    ...
    'formula', 'Income - (Unpaid Fixed + MAX(Variable Actual, Variable Planned × Remaining Days) + Unpaid Investments + Credit Cards)',
