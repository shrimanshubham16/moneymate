...
DROP POLICY IF EXISTS "Users can view own variable_plans" ON variable_expense_plans;
CREATE POLICY "Users can view own variable_plans" ON variable_expense_plans
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own variable_plans" ON variable_expense_plans;
CREATE POLICY "Users can insert own variable_plans" ON variable_expense_plans
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own variable_plans" ON variable_expense_plans;
CREATE POLICY "Users can update own variable_plans" ON variable_expense_plans
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own variable_plans" ON variable_expense_plans;
...
DROP POLICY IF EXISTS "Users can view own variable_actuals" ON variable_expense_actuals;
CREATE POLICY "Users can view own variable_actuals" ON variable_expense_actuals
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own variable_actuals" ON variable_expense_actuals;
CREATE POLICY "Users can insert own variable_actuals" ON variable_expense_actuals
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own variable_actuals" ON variable_expense_actuals;
CREATE POLICY "Users can update own variable_actuals" ON variable_expense_actuals
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own variable_actuals" ON variable_expense_actuals;
...
DROP POLICY IF EXISTS "Users can view own future_bombs" ON future_bombs;
CREATE POLICY "Users can view own future_bombs" ON future_bombs
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own future_bombs" ON future_bombs;
CREATE POLICY "Users can insert own future_bombs" ON future_bombs
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own future_bombs" ON future_bombs;
CREATE POLICY "Users can update own future_bombs" ON future_bombs
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own future_bombs" ON future_bombs;
...
DROP POLICY IF EXISTS "Users can view own constraint_score" ON constraint_scores;
CREATE POLICY "Users can view own constraint_score" ON constraint_scores
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own constraint_score" ON constraint_scores;
...
DROP POLICY IF EXISTS "Users can view own health_cache" ON health_cache;
CREATE POLICY "Users can view own health_cache" ON health_cache
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own health_cache" ON health_cache;
CREATE POLICY "Users can insert own health_cache" ON health_cache
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own health_cache" ON health_cache;
...
DROP POLICY IF EXISTS "Users can view shared_accounts they belong to" ON shared_accounts;
...
DROP POLICY IF EXISTS "Users can view shared_members of their accounts" ON shared_members;
...
DROP POLICY IF EXISTS "Users can view own sharing_requests" ON sharing_requests;
CREATE POLICY "Users can view own sharing_requests" ON sharing_requests
  FOR SELECT USING (auth.uid() = inviter_id OR auth.uid() = invitee_id);

DROP POLICY IF EXISTS "Users can insert sharing_requests" ON sharing_requests;
CREATE POLICY "Users can insert sharing_requests" ON sharing_requests
  FOR INSERT WITH CHECK (auth.uid() = inviter_id);

DROP POLICY IF EXISTS "Users can update sharing_requests they received" ON sharing_requests;
...
DROP POLICY IF EXISTS "Users can view own theme_state" ON theme_states;
CREATE POLICY "Users can view own theme_state" ON theme_states
  FOR SELECT USING (auth.uid() = owner_ref);

DROP POLICY IF EXISTS "Users can insert own theme_state" ON theme_states;
CREATE POLICY "Users can insert own theme_state" ON theme_states
  FOR INSERT WITH CHECK (auth.uid() = owner_ref);

DROP POLICY IF EXISTS "Users can update own theme_state" ON theme_states;
