...
describe("MoneyMate Functional Tests (QA)", () => {
  ...
  describe("AUTH - Signup & Login", () => {
    ...
    it("should reject duplicate email", async () => {
      const res = await request(app)
        .post("/auth/signup")
        .send({ email: "user1@test.com", password: "pass", username: "different" });
    ...
    it("should reject invalid credentials", async () => {
      const res = await request(app)
        .post("/auth/login")
        .send({ email: "user1@test.com", password: "wrong" });
  ...
  describe("PLANNING - Variable Expenses", () => {
    ...
    it("should require justification for overspend in red tier", async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body.error.message).toContain("justification");
